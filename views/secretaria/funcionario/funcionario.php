...
if ($s[0]['FOTO'] != '') { ?>
<?= $s[0]['CHAPA_COM_DV'] . $s[0]['VIA'] ?>   <?= str_pad($p->NM_FUNCIONARIO,40," ",STR_PAD_RIGHT) ?> PROFESSOR(A)   <?= str_pad($s[0]['RG'],15," ",STR_PAD_RIGHT) ?><?= str_pad($s[0]['CPF'],14," ",STR_PAD_RIGHT) ?><?= str_pad($s[0]['CTPS'],15," ",STR_PAD_RIGHT) ?><?= str_pad($s[0]['VIA'],5," ",STR_PAD_RIGHT) ?><?= str_pad(date('d/m/Y', strtotime($s[0]['DT_ADMISSAO'])),10," ",STR_PAD_RIGHT) ?>         
<?}}?>
<? exit(); ?>