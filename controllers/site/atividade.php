...
class Atividade extends CI_Controller {
    ...
    function index() { 
        if($this->input->post('acao') == 1){
            $dados = array('operacao'=>'LA' );
            $atividade = $this->site->sp_site($dados);
            ...
            if(count($atividade[0]['CD_ATIVIDADE']) == 0){
                $dados1['operacao'] = 'IA';
            ...
            redirect(base_url('site/atividade/index'), 'refresh');
        }else{
            $data['titulo'] = '<h1> Site <i class="fa fa-angle-double-right"></i> Dados das Atividades</h1>';
            $dados2 = array('operacao'=>'LA');
            $data['atividade'] = $this->site->sp_site($dados2);

            $this->load->view('site/atividade/cadastro_atividade',$data);
        }
    }
}